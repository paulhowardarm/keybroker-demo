...
use base64::prelude::*;
use keybroker_common::{BackgroundCheckKeyRequest, PublicWrappingKey, WrappedKeyData};
...
fn main() {
    ...
    // Get ready to make HTTP post calls - use a blocking client for simplicity
    let client = reqwest::blocking::Client::new();

    // Make the first API call to request the key. Assume the server is running on the localhost.
    // on port 8088, and that we are requesting the key called "skywalker".
    let res = client
        .post("http://127.0.0.1:8088/keys/v1/key/skywalker")
        .json(&key_request)
        .send()
        .unwrap();
    ...
    if let Some(l) = location {
        let evidence = URL_SAFE_NO_PAD.encode(CCA_EXAMPLE_TOKEN);
        let resp = client
            .post(l.to_str().unwrap())
            .header(
        ...
        let wrapped_data: WrappedKeyData = resp.json().unwrap();

        let ciphertext = URL_SAFE_NO_PAD
            .decode(wrapped_data.data)
            .expect("Failed to base64-decode the wrapped data from the server.");

        let plaintext = priv_key
            .decrypt(Pkcs1v15Encrypt, &ciphertext)
            .expect("Failed to decrypt wrapped data from the server.");

        let plainstring = String::from_utf8(plaintext).unwrap();

        println!(
            "The key returned from the server decodes to: {}",
            plainstring
        );
    }
}
